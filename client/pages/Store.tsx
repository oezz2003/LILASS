...
type Product = ProductInput & { id: string; addedAt: string };
...
export default function Store() {
  ...
  const onSubmit = (values: ProductInput) => {
    if (editingId) {
      updateMutation.mutate({ id: editingId, data: { ...values, imageUrl: (values.imageUrl ?? "").trim() } });
      setEditingId(null);
      form.reset({ ...form.getValues(), name: "", size: "", price: 0, sku: "", stock: 0, description: "", imageUrl: "" });
      return;
    }
    createMutation.mutate({ ...values, imageUrl: (values.imageUrl ?? "").trim() });
                    ...
                    <TableCell className="text-right whitespace-nowrap">
                      <div className="flex justify-end gap-1">
                        ...
                        <Button
                          ...
                          onClick={() => { deleteMutation.mutate(p.id); if (editingId === p.id) { setEditingId(null); form.reset(); } }}
