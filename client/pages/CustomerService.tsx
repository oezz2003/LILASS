...
export default function CustomerService() {
  ...
  // Move filters above query to avoid use-before-declaration
  ...
  const feedbackQuery = useQuery({
    queryKey: ["cs-feedback", scale, year, quarter, effectiveMonth, week, typeFilter, query],
    queryFn: async () => {
      ...
      const res = await fetch(`/api/cs/feedback?${params.toString()}`);
      if (!res.ok) throw new Error("Failed to load feedback");
      return (await res.json()) as { feedback: Feedback[] };
    },
  });
  const feedbacks = feedbackQuery.data?.feedback ?? [];
