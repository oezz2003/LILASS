...
function CostSection({ title, storageKey, description }: CostSectionProps) {
  ...
  const section = storageKey.includes("cogs") ? "cogs" : storageKey.includes("tech") ? "tech" : "variable";
  const costsQuery = useQuery({
    queryKey: ["finance-costs", section, scale, year, quarter, effectiveMonth, week],
    queryFn: async () => {
      ...
      const res = await fetch(`/api/finance/costs?${params.toString()}`);
      if (!res.ok) throw new Error("Failed to load costs");
      return (await res.json()) as { entries: { id: string; date: string; amount: number; note: string }[]; buckets: { label: string; total: number }[]; total: number };
    },
  });
  const createCostMutation = useMutation({
    mutationFn: async (payload: { section: string; date: string; amount: number; note?: string }) => {
  ...
  const deleteCostMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await fetch(`/api/finance/costs/${id}`, { method: "DELETE" });
  ...
  function addEntry() {
    ...
    createCostMutation.mutate({ section, date, amount: Math.round(amt * 100) / 100, note: note.trim() });
    setAmount("");
    setNote("");
  }
  ...
  const data: Bucket[] = useMemo(() => {
    if (costsQuery.data?.buckets) return costsQuery.data.buckets as any;
    if (scale === "yearly") {
  ...
  const total = costsQuery.data?.total ?? useMemo(() => data.reduce((a, b) => a + b.total, 0), [data]);
        ...
        <div className="mt-4">
          ...
          <div className="max-h-64 overflow-auto rounded-md border">
            <Table>
              ...
              <TableBody>
                {(costsQuery.data?.entries ?? rangedEntries).map((e) => (
                  <TableRow key={e.id}>
                    <TableCell>
...
export default function Finance() {
  ...
  const invoicesQuery = useQuery({
    queryKey: ["finance-invoices", scaleI, yearI, quarterI, effectiveMonthI, weekI],
    queryFn: async () => {
      const params = new URLSearchParams();
      params.set("scale", scaleI);
      params.set("year", String(yearI));
      params.set("quarter", String(quarterI));
      params.set("month", String(effectiveMonthI));
      params.set("week", String(weekI));
  ...
  const filteredInvoices = invoicesQuery.data?.invoices ?? [];
